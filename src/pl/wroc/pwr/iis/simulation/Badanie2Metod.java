...
public abstract class Badanie2Metod {
	...
	private int iloscEksperymentow = 1;
	...
	protected void setParametryEksperymentu(int iloscIteracji, int iloscEksperymentow, int pomiarCo) {
		setIloscIteracji(iloscIteracji);
		this.iloscEksperymentow = iloscEksperymentow;
		this.pomiarCo = pomiarCo;
	}
	
	protected void setParametryEksperymentu(int iloscIteracji, int iloscEksperymentow, int pomiarCo, int pomiarOd, int pomiarDo) {
		setParametryEksperymentu(iloscIteracji, iloscEksperymentow, pomiarCo);
	...
	protected void runSerwer(Serwer wybranySerwer, double[][] wyniki, int numerEksperymentu) {
		...
		for (int iteracja = 0; iteracja < iloscIteracji; iteracja++) {
			...
			wybranySerwer.wykonajCyklSymulacji();
...
//				System.out.println(out.toString());
			...
			if (iteracja > pomiarOd && iteracja < pomiarDo){
				for (int k = 0; k < wybranySerwer.getIloscKolejek(); k++) {
					float sredniCzasOczekiwania = wybranySerwer.getKolejka(k).getSredniCzasOczekiwania();
				...
				if (wybranySerwer.getSterownik().bylShake()) {
					srednieCzastkowe[srednieCzastkowe.length-1] += 1;	
				}
